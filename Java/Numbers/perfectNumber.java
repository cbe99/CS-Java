...
What is the perfect number?
    ...
    Proper positive divisors are positive integers that fully divide 
...
EXAMPLE INPUT/OUTPUT:
    * isPerfectNumber(6); should return true since its proper divisors are 
        1, 2, 3 and the sum is 1 + 2 + 3 = 6
    * isPerfectNumber(28); should return true since its proper divisors are 
        1, 2, 4, 7, 14 and the sum is 1 + 2 + 4 + 7 + 14 = 28
    * isPerfectNumber(5); should return false since 
        its only proper divisor is 1 and the sum is 1 not 5
...
public class perfectNumber {
    public static void main(String[] args) {
        ...
        System.out.println(isPerfectNumber(2896));
    ...
    public static boolean isPerfectNumber(int number) {
        if (number < 1)
            return false;
        int sum = 1;
        for (int i = 2; i < Math.sqrt(number); i++) { // i = 1; i < 6; 6%1 = 0 > sum = 0+1 = 1 >> i = 2 6%2=0, 1 + 2 =
                                                      // 3; 6%3 = 0 3 + 3 = 6
            if (number % i == 0) {
                sum += (i + number / i);
            }
